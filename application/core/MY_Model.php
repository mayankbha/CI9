...
class MY_Model extends CI_Model {
    ...
    function get($type = "all", $order_field = '', $order = 'ASC', $limit = false, $offset = '',$joins=array(),$select=false,$groupby=false) {
		if(!$select){
			$this->db->select($this->tbl.'.*',false);
		}else{
	...
	{
		$this->db->select($this->tbl.'.'.$this->primary_key,false);
 		if (is_array($type) && !empty($type)) {
			foreach ($type as $keys => $vals) {
				if($vals!==false){
					if(!is_array($vals)){
